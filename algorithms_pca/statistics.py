...
# Центрируем данные по столбцам: вычитаем среднее
def center_data(X: Matrix) -> Tuple[Matrix, List[float]]:
    n, m = X.rows, X.cols
    means: List[float] = [sum(X._data[i][j] for i in range(n)) / n for j in range(m)]
    centered = [[X._data[i][j] - means[j] for j in range(m)] for i in range(n)]
